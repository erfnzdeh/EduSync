...
class QueraCalendarBot:
    def __init__(self, token: str):
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                MAIN_MENU: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_menu_selection)],
                ...
                SYNC_MENU: [MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_sync_menu_selection)],
            },
    ...
    async def handle_menu_selection(self, update: Update, context: CallbackContext) -> int:
        ...
        if selection == SYNC_OPTIONS:
            if not user_data.get('quera_session_id') or not user_data.get('gcal_connected', False):
                await update.message.reply_text(
    ...
    async def sync_calendars(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            ...
            success_count = results['created'] + results['updated'] + results['existing']
    ...
    def get_main_keyboard(self, user_id: str = None) -> ReplyKeyboardMarkup:
        ...
        keyboard.append([
            KeyboardButton(DISCONNECT_GCAL if has_gcal else CONNECT_GCAL)
        ])
    ...
    async def handle_sync_menu_selection(self, update: Update, context: CallbackContext) -> int:
        """Handle sync menu selections."""
        user_id = str(update.effective_user.id)
        selection = update.message.text.replace(" ✅", "").replace(" ❌", "")  # Remove status indicators
        user_data = self.user_data.get(user_id, {})
